// Commands executed in a GLOBAL scope, e.g. created hitograms aren't erased...
{
  ...
  TBranch * branchLayer = tree->GetBranch("PHcalValidInfoLayer_r__CaloTest.obj");
  assert(branchLayer != 0);

  TBranch * branchNxN = tree->GetBranch("PHcalValidInfoNxN_r__CaloTest.obj");
  assert(branchNxN != 0);

  TBranch * branchJets = tree->GetBranch( "PHcalValidInfoJets_r__CaloTest.obj");  assert(branchJets != 0);
